document.addEventListener('DOMContentLoaded', () => {
    ...
    const fetchProducts = async () => {
        try {
            ...
            const res = await fetch('api/products').catch(() => fetch('data.json'));
            if (!res.ok && res.url.includes('api')) return fetch('data.json').then(r => r.json());
    ...
    const fetchCategories = async () => {
        try {
            const res = await fetch('api/categories').catch(() => fetch('categories.json'));
            if (!res.ok && res.url.includes('api')) return fetch('categories.json').then(r => r.json());
            return await res.json();
        } catch (err) {
